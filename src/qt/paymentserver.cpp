...
const int BBQCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BBQCOIN_IPC_PREFIX("bbqcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(BBQCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BBQCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(BBQCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click bbqcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start bbqcoin: click-to-pay handler");
    else
...
{
    // clicking on bbqcoin: URLs creates FileOpen events on the Mac:
