...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 5); // BBQCoin: starting difficulty is 1 / 2^12
...
{
    // BBQCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 m_nV100500Height = 490845;

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 42 * COIN;

    if (nHeight == m_nV100500Height) { 
        nSubsidy = 351582 * COIN;
    } else {
        nSubsidy >>= (nHeight / 2400000);
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BBQCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfd, 0xe4, 0xd9, 0x42 }; // BBQCoin: increase each by adding 2 to bbqcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BBQCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BBQCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BBQCoinMiner(CWallet *pwallet)
{
    printf("BBQCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bbqcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BBQCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BBQCoinMiner terminated\n");
        throw;
    }
}

void GenerateBBQCoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BBQCoinMiner, pwallet));
}
